...
void pass(int semid, int pass_id)
{
 ...
 while (1) {
  { struct sembuf command__LINE__ = {0, -1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM P #__LINE__"); };
  { struct sembuf command__LINE__ = {2, -1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM P #__LINE__"); };
  printf("[%d] Иду по трапу на борт\n", pass_id);
  { struct sembuf command__LINE__ = {2, 1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM V #__LINE__"); };
  { struct sembuf command__LINE__ = {1, 1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM V #__LINE__"); };
  { struct sembuf command__LINE__ = {5, -1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM P #__LINE__"); };
  printf("[%d] Ура, меня покатали!\n", pass_id);
  { struct sembuf command__LINE__ = {6, 1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM V #__LINE__"); };
  { struct sembuf command__LINE__ = {3, -1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM P #__LINE__"); };
  printf("[%d] Иду по трапу в сторону берега\n", pass_id);
  { struct sembuf command__LINE__ = {3, 1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM V #__LINE__"); };
  { struct sembuf command__LINE__ = {1, 1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM V #__LINE__"); };
 }
}
...
# 71 "hohlocode.c" 3 4
 16
# 71 "hohlocode.c"
 , 0) == -1) perror("SETVAL #__LINE__");;
  if (semctl(semid, 0, 
# 72 "hohlocode.c" 3 4
 16
# 72 "hohlocode.c"
 , 4) == -1) perror("SETVAL #__LINE__");;
  { struct sembuf command__LINE__ = {2, 1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM V #__LINE__"); };

  for (int i = 0; i < 4; i++)
   { struct sembuf command__LINE__ = {1, -1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM P #__LINE__"); };
...
# 83 "hohlocode.c"
 , 4) == -1) perror("SETVAL #__LINE__");;
  ...
  for (int i = 0; i < 4; i++)
   { struct sembuf command__LINE__ = {6, -1, 0}; if (semop(semid, &command__LINE__, 1) == -1) perror("SEM P #__LINE__"); };
  printf("Выгружаемся\n");
  if (semctl(semid, 1, 
# 88 "hohlocode.c" 3 4
 16
...
# 113 "hohlocode.c" 3 4
                                  16
# 113 "hohlocode.c"
                                        , 0) == -1) perror("GOD::SET SEM_GO_HOME");
